...
menu_archivo.add_command(label="Nuevo", command=lambda: archivo_nuevo(texto, root, barra_estatus))
menu_archivo.add_command(label="Abrir", command=lambda: archivo_abrir(texto, root, barra_estatus))
menu_archivo.add_command(label="Guardar", command=lambda: grabararchivo(texto, name, barra_estatus))
menu_archivo.add_command(label="Guardar como", command=lambda: grabarcomo(texto, root, barra_estatus))
...
menu_editar.add_command(label="Cortar   Ctrl+X", command=lambda: cortartexto(texto, False))
menu_editar.add_command(label="Copiar   Ctrl+C", command=lambda: copiartexto(root, texto, False))
menu_editar.add_command(label="Pegar   Ctrl+V", command=lambda: pegartexto(root, texto, seleccionado, False))
...
menu_editar.add_command(label="Buscar", command=lambda: buscar_texto(texto), accelerator="Ctrl+B")
menu_editar.add_command(label="Reemplazar", command=lambda: reemplazar_texto(texto), accelerator="Ctrl+R")
...
# Botones
cambiofondo_texto = Button(marco_de_botones, text="Cambiar color de fondo", command=lambda: cambiocolorfondo(texto))
cambiofondo_texto.grid(row=0, column=2)

cambiotodo_texto = Button(marco_de_botones, text="Cambiar color predeterminado texto", command=lambda: cambiocolortextopredeterminado(texto))
